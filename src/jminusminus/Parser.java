...
public class Parser {
	...
	private boolean seeLabel() { // Modified HW3
		scanner.recordPosition();
		boolean result = see(COLON);
		scanner.returnToPosition();
		return result;
	}
	...
	private JAST typeDeclaration() { // Modified HW3
		// Only handled every TypeDeclaration as a class so I tried to make it
		...
		if (see(CLASS)) {
			...
			mustBe(SEMI);
			return new JEmptyStatement(line); // if its not class or interface
		}
	...
	private JClassDeclaration classDeclaration(ArrayList<String> mods) {
		...
		// Modified HW3
		ArrayList<TypeName> implement = new ArrayList<TypeName>();
		if (have(IMPLEMENTS)) {
			do {
		...
		return new JClassDeclaration(line, mods, name, superClass, implement,
				classBody());
	}
	...
	private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
		...
		String name = scanner.previousToken().image();
		// Type superClass;
		ArrayList<TypeName> extend = new ArrayList<TypeName>();
		if (have(EXTENDS)) {
			do {
				extend.add(qualifiedIdentifier());
			} while (have(COMMA));
		}
		return new JInterfaceDeclaration(line, mods, name, extend,
				interfaceBody());
	}
	...
	private ArrayList<JMember> classBody() {
		...
		while (!see(RCURLY) && !see(EOF)) {
			if (have(SEMI)) {
				int line = scanner.token().line();
				members.add(new JEmptyMember(line));
			} else
	...
	private JMember memberDecl(ArrayList<String> mods) {
		...
		if (see(CLASS)) { // inner class
			memberDecl = classDeclaration(mods);
		} else if (see(INTERFACE)) { // inner interface
			...
			memberDecl = new JConstructorDeclaration(line, mods, name, params,
					body);
		} else if (have(LCURLY) && mods.size() == 1 && mods.contains(STATIC.image())) { // Modified HW3 
			JBlock body = have(SEMI) ? null : block();
			memberDecl = new JStaticBlock(line, mods, body);
		} else if (have(LCURLY)) { // Modified HW3 
			JBlock body = have(SEMI) ? null : block();
			memberDecl = new JInstanceBlock(line, body);
		} else {
			Type type = null;
			if (have(VOID)) {
				// void method
	...
	private JMember interfaceMemberDecl(ArrayList<String> mods) {
		...
		Type type = null;
		if (see(CLASS)) { // for inner classes
			interfaceMemberDecl = classDeclaration(mods);
		} else if (see(INTERFACE)) { // for inner interfaces
			interfaceMemberDecl = interfaceDeclaration(mods);
		} else if (seeIdentLParen()) {
			// A constructor
			mustBe(IDENTIFIER);
			String name = scanner.previousToken().image();
			ArrayList<JFormalParameter> params = formalParameters();
			JBlock body = block();
			interfaceMemberDecl = new JConstructorDeclaration(line, mods, name,
					params, body);
		} else if (have(VOID)){ // Modified HW3
				// void method
				type  = Type.VOID;
				mustBe(IDENTIFIER);
				String name = scanner.previousToken().image();
				ArrayList<JFormalParameter> params = formalParameters();
				JBlock body = null; // empty body
				interfaceMemberDecl = new JMethodDeclaration(line, mods, name,
						type, params, body);
			} else {
				type = type();
				if (seeIdentLParen()) {
					...
					ArrayList<JFormalParameter> params = formalParameters();
					JBlock body = have(SEMI) ? null : block();
					interfaceMemberDecl = new JMethodDeclaration(line, mods,
	...
	private JStatement statement() {
		...
		} else if (have(BREAK)) { // Modified HW3
			JExpression expr = expression();
			return new JBreakStatement(line, expr);
		} else if (have(SEMI)) {
	...
	private Type basicType() {
		if (have(BOOLEAN)) {
			...
			return Type.INT;
		} else if (have(DOUBLE)) { // Modified HW3
			return Type.DOUBLE;
		} else {
	...
	private JStatement statementExpression() {
		...
		if (expr instanceof JAssignment
				...
				|| expr instanceof JPreDecrementOp // Modified HW3
				|| expr instanceof JPostIncrementOp // Modified HW3
	...
	private JExpression assignmentExpression() {
		int line = scanner.token().line();
		JExpression lhs = conditionalExpression(); // Modified HW3
		if (have(ASSIGN)) {
			return new JAssignOp(line, lhs, assignmentExpression());
		} else if (have(PLUS_ASSIGN)) {
			return new JPlusAssignOp(line, lhs, assignmentExpression());
		} else if (have(MINUS_ASSIGN)) { // Modified HW3
			return new JMinusAssignOp(line, lhs, assignmentExpression());
		} else if (have(STAR_ASSIGN)) { // Modified HW3
			return new JStarAssignOp(line, lhs, assignmentExpression());
		} else if (have(DIV_ASSIGN)) { // Modified HW3
	...
	private JExpression conditionalExpression() { // Modified HW3
		int line = scanner.token().line();
		boolean more = true;
		...
		while (more) {
			...
			} else if (have(LOR)) { // Modified HW3
				lhs = new JLogicalOrOp(line, lhs, equalityExpression());
			} else if (have(QMARK)) { // Modified HW3
				JExpression consequent = assignmentExpression();
				// ternary if (?) will not work without ternary else (:)
				mustBe(COLON);
	...
	private JExpression primary() {
		...
		if (see(LPAREN)) {
			...
			if (see(LPAREN)) {
				...
				return new JFieldSelection(line, ambiguousPart(id), null,
						id.simpleName());
			}
		} else {
			return literal();
	...
	private JExpression literal() {
		...
		} else if (have(DOUBLE_LITERAL)) { // Modified HW3
